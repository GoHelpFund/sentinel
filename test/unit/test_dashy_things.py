...
def test_valid_help_address():
    from helplib import is_valid_help_address

    main = valid_help_address()
    test = valid_help_address('testnet')

    assert is_valid_help_address(main) is True
    assert is_valid_help_address(main, 'mainnet') is True
    assert is_valid_help_address(main, 'testnet') is False

    assert is_valid_help_address(test) is False
    assert is_valid_help_address(test, 'mainnet') is False
    assert is_valid_help_address(test, 'testnet') is True


def test_invalid_help_address():
    from helplib import is_valid_help_address

    main = invalid_help_address()
    test = invalid_help_address('testnet')

    assert is_valid_help_address(main) is False
    assert is_valid_help_address(main, 'mainnet') is False
    assert is_valid_help_address(main, 'testnet') is False

    assert is_valid_help_address(test) is False
    assert is_valid_help_address(test, 'mainnet') is False
    assert is_valid_help_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from helplib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % helplib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(helplib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(helplib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(helplib.blocks_to_seconds(16616)) == 2612035
