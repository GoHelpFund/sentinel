...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, helpd):
        golist = helpd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_helpd(helpd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_helpd(self, helpd, rec):
        import decimal
        import helplib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from helpd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from helpd, with every run
        ...
        # get/create, then sync payment amounts, etc. from helpd - Helpd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(helpd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from helpd! %s" % e)
            govobj.vote_delete(helpd)
            return (govobj, None)
    ...
    def vote_delete(self, helpd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(helpd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, helpd, signal, outcome):
        import helplib
        ...
        output = helpd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = helplib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(helpd, signal)

    def sync_network_vote(self, helpd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = helpd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = HELPD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 help addr, non-multisig
            if not helplib.is_valid_help_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Help address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Help Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (helplib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = HELPD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not helplib.is_valid_help_address(addr, config.network):
    ...
    def hash(self):
        import helplib
        return helplib.hashit(self.serialise())
