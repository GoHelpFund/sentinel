...
class HelpConfig():
    ...
    def slurp_config_file(self, filename):
        # read help.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is help.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        # convert to an int if taken from help.conf
