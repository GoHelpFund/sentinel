...
def is_valid_help_address(address, network='mainnet'):
    ...
    help_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Help address
        return False

    if (address_version != help_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('help.conf')
