...
def has_help_conf():
    ...
    valid_help_conf = False

    # ensure help_conf exists & readable
    #
    # if not, print a message stating that Help Core must be installed and
    # configured, including JSONRPC access in help.conf
    try:
        f = io.open(config.help_conf)
        valid_help_conf = True
    except IOError as e:
        print(e)

    return valid_help_conf
...
def main():
    ...
    if not has_help_conf():
        print("HelpCore must be installed and configured, including JSONRPC access in help.conf")
        sys.exit(1)
