...
class HelpDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some helpd methods
    ...
    @classmethod
    def from_help_conf(self, help_dot_conf):
        from help_config import HelpConfig
        config_text = HelpConfig.slurp_config_file(help_dot_conf)
        creds = HelpConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = helplib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = helplib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = helplib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_helpd_proto_version_with_sentinel_ping)
